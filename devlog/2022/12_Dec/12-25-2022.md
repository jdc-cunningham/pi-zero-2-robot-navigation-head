...
almost got the rotation point wrong for panning

<img src="../../images/object-modeling.JPG"/>

How it looks in SketchUp

<img src="../../images/obstacle-in-view.JPG"/>

Here you can see at 21.2 deg the box is completely in view of the tof FOV

I don't have a way to point the servos right now to some angle... need to calibrate that

hardcoded with external verification (camera/protractor) is easiest right now

5:03 PM

I wrote some control code for the head somewhere since the web interface uses it

oh it was doing 10 ms increments for pwm

I'm kind of spent now... hmmm

I need to do math for beam pointing next

Make servo commands that can take a degree setting

tomorrow is a binge day (hopefully, means got past my car starting issues if any) so I probably won't be productive

I'll try, will mostly chill though

Right now I feel like I have not gotten much done it's sad ha

Let me get a quick measurement even with manual code

I don't know if there's a direct translation between degrees to pwm mathematically/mechanically, I'll see

Probably with regard to the servo's design but idk it

there is something that's nice, the purple patch from the lidar can tell you what you're measuring... an electrically controlled laser pointer would be good actually other than potentially being blinding to creatures

I actually could verify this... with the robot take a photo before taking measurement

see if I'm aiming at the right thing... since the tof is right next to the camera... but the tof FOV is so big... will need to do a stereo overlap check

well... not really a thing since they just join together anyway the two fovs

ahh man sucks... but I'm also losing drive now

I know what I need to do next anyway

I will need to setup a rig/setup where the camera can look at the rotation axis head on from the side and from the top, and then I'll do some calibration settings to get a deg to ms conversion

0 to 2500, for max rotation, can divide to get degrees probably

will have to see if I have to worry about mechanical limitation/rotates too far.

for tilt that is a thing

I'll determine those manually, I need a web interface so I don't have to keep programming/git push/pull or at least write code remotely by ssh
